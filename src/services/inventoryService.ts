...
import type { Ingredient, IngredientBatch, Event, Menu, Recipe, InventoryItem } from '../types';
...
export const deductStockForEvent = async (eventId: string, _userId?: string): Promise<void> => {
    ...
    for (const [ingId, qtyNeeded] of Object.entries(ingredientNeeds)) {
        ...
        const inventoryItems = await firestoreService.query<InventoryItem>(collections.inventory,
